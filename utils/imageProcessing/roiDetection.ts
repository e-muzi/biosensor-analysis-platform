...
export function validateROI(
  ...
  if (roi.x < 0 || roi.x > 1 || roi.y < 0 || roi.y > 1) {
    return {
      isValid: false,
      errorMessage: 'ROI coordinates must be between 0 and 1',
    };
  }
  ...
  if (roi.width <= 0 || roi.height <= 0) {
    return {
      isValid: false,
      errorMessage: 'ROI width and height must be positive',
    };
  }
  ...
  if (roi.x + roi.width > 1 || roi.y + roi.height > 1) {
    return {
      isValid: false,
      errorMessage: 'ROI extends beyond canvas boundaries',
    };
  }
  ...
  if (pixelWidth < 1 || pixelHeight < 1) {
    return {
      isValid: false,
      errorMessage: 'ROI is too small (less than 1 pixel)',
    };
  }
