          ...
          return (
            <Box key={result.pesticide}>
              ...
              >
                ...
                >
                  {result.pesticide}{' '}
                  {result.validPixels === 0 ? '⚠️' : `(${result.validPixels})`}
                </Typography>
              </Box>
            </Box>
          );
            ...
            <Collapse in={isPanelExpanded}>
                        ...
                        .map((pixel, pixelIndex) => {
                          ...
                          return (
                            <Box key={pixelIndex} mb={0.3}>
                              ...
                              >
                                {isTopPixel ? '🏆 ' : ''}#{pixelIndex + 1}: RGB(
                                {pixel.r},{pixel.g},{pixel.b}) B:
                                {pixel.brightness.toFixed(2)}
                              </Typography>
                            </Box>
                          );
                        })
              ...
              {samplingResults.length > 0 && (
                <Box mb={3}>
                  <Typography
                    variant='subtitle1'
                    sx={{ color: '#4fc3f7', fontWeight: 'bold', mb: 1 }}
                  >
                  ...
                  {samplingResults.map(result => {
                    ...
                    const averageR =
                      pixels.length > 0
                        ? pixels.reduce((sum, p) => sum + p.r, 0) /
                          pixels.length
                        : 0;
                    const averageG =
                      pixels.length > 0
                        ? pixels.reduce((sum, p) => sum + p.g, 0) /
                          pixels.length
                        : 0;
                    const averageB =
                      pixels.length > 0
                        ? pixels.reduce((sum, p) => sum + p.b, 0) /
                          pixels.length
                        : 0;
                    ...
                    return (
                      ...
                      >
                        <Typography
                          variant='subtitle2'
                          sx={{ color: '#4fc3f7', fontWeight: 'bold', mb: 0.5 }}
                        >
                          {result.pesticide}
                        </Typography>
                        ...
                        >
                          5-pixel sampling (center + 4 nearby) at absolute
                          coordinates
                        </Typography>
                          ...
                          <Box ml={1}>
                            ...
                            {pixels.slice(0, 3).map((pixel, pixelIndex) => (
                              <Typography
                                ...
                                sx={{
                                  color:
                                    pixelIndex === 0 ? '#4fc3f7' : '#e0e0e0',
                                  fontSize: '0.6rem',
                                  display: 'block',
                                  lineHeight: 1.2,
                                }}
                              >
                                {pixelIndex === 0 ? '🏆 ' : '  '}RGB({pixel.r},
                                {pixel.g},{pixel.b}) B:
                                {pixel.brightness.toFixed(1)}
                              </Typography>
                            ))}
                            {pixels.length > 3 && (
                              <Typography
              ...
              {samplingResults.map(result => (
                <Box key={result.pesticide} mb={2}>
                  <Typography
                    variant='subtitle1'
                    sx={{ color: '#81c784', fontWeight: 'bold' }}
                  >
                    {result.pesticide}
                  </Typography>
                  ...
                  <Box display='flex' gap={0.5} mb={1} flexWrap='wrap'>
                    ...
                    />
                    {(result.pesticide === 'Acephate' ||
                      result.pesticide === 'Atrazine') && (
                      <Chip
                        label='Edge'
                        size='small'
                        sx={{
