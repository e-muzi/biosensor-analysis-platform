...
export function CalibrationSettings() {
  ...
  const handleEditSave = () => {
    ...
    try {
      ...
      setError(
        'Invalid concentration values. Please use comma-separated numbers.'
      );
    }
  };
      ...
      <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
            ...
            <CardContent>
                  ...
                  <Box sx={{ display: 'flex', gap: 1 }}>
                    {editingPesticide === pesticide.name ? (
                      ...
                      <>
                        <Button
                          size='small'
                          onClick={() => handleEditStart(pesticide.name)}
                        >
                          Edit
                        </Button>
                        <Button
                          size='small'
                          onClick={() => handleReset(pesticide.name)}
                          color='warning'
                        >
                          Reset
                        </Button>
              ...
              {detectionMode === 'preset' ? (
                <Box>
                  ...
                  >
                    This curve is predefined and cannot be edited. Edit the code
                    to modify preset curves.
                  </Typography>
                </Box>
              ) : (
                <>
                  {editingPesticide === pesticide.name ? (
                    ...
                    <Typography variant='body2' color='text.secondary'>
                      ...
                      {userCalibrations[pesticide.name]?.join(', ') ||
                        'Not set'}
                    </Typography>
                  )}
                </>
        ...
        {detectionMode === 'strip' && (
          <Box sx={{ mt: 3, display: 'flex', justifyContent: 'center' }}>
            <AppButton variant='outline' color='warning' onClick={resetAll}>
